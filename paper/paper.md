...
**Step 1: Filtering by global sensitivity analysis.** We first perform a mutual information-based global sensitivity analysis to exclude inputs sharing zero information with $Y_j$ and narrow down a subset $\mathbf{X}^{S_1}_j$ such that:
...
where $\mathbf{X}^{S_1}_j \backslash X_i$ is the remaining set of $\mathbf{X}^{S_1}_j$ by excluding $X_i$; $I(X_i;Y_j|\mathbf{X}^{S_1}_j \backslash X_i)$ is the conditional mutual information between $X_i$ and $Y_j$ conditioning on $\mathbf{X}^{S_1}_j \backslash X_i$. $I(X_i;Y_j|\mathbf{X}^{S_1}_j \backslash X_i) = 0$ indicates that $X_i$ and $Y_j$ are independent given the knowledge of $\mathbf{X}^{S_1}_j \backslash X_i$. Directly conditioning on the full $\mathbf{X}^{S_1}_j \backslash X_i$ faces the curse of dimensionality. To address this issue, we approximate it by conditioning on only the $k$ most sensitive remaining members of $\mathbf{X}^{S_1}_j \backslash X_i$ for increasing $k$ up to a maximum cardinality (i.e., three by default), removing $X_i$ from $\mathbf{X}^S_j$ as soon as one such test finds independence; see the online documentation for the full algorithmic detail.
