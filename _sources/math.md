...
where $\mathbf{X}^{S_1}_j \backslash X_i$ is the remaining set of $\mathbf{X}^{S_1}_j$ by excluding $X_i$; $I(X_i;Y_j|\mathbf{X}^{S_1}_j \backslash X_i)$ is the conditional mutual information between $X_i$ and $Y_j$ conditioning on $\mathbf{X}^{S_1}_j \backslash X_i$. $I(X_i;Y_j|\mathbf{X}^{S_1}_j \backslash X_i) = 0$ indicates that $X_i$ and $Y_j$ are independent given the knowledge of $\mathbf{X}^{S_1}_j \backslash X_i$. However, calculating $\mathbf{X}^{S_1}_j \backslash X_i$. $I(X_i;Y_j|\mathbf{X}^{S_1}_j \backslash X_i)$ faces the curse of dimensionality due to the potential high dimension in $\mathbf{X}^{S_1}_j$. To address this, we leverage the idea of Peter-Clark algorithm for causal inference detection {cite:p}`Spirtes:2001` to evaluate the zeroness of a high-dimensional conditional mutual information by gradually adding conditioning variables. Specifically, we approximated $I(X_i;Y_j|\mathbf{X}^{S_1}_j \backslash X_i)$ via $I(X_i;Y_j|\mathbf{X}^{S_2}_j)$, where $\mathbf{X}^{S_2}_j$ is a subset of $\mathbf{X}^{S_1}_j \backslash X_i$ with a predefined maximum cardinality $K$ (i.e., three by default). Rather than exhausting every subset of a given cardinality, $\mathbf{X}^{S_2}_j$ is chosen greedily as the $k$ members of $\mathbf{X}^{S_1}_j \backslash X_i$ with the highest Step 1 sensitivity, for $k = 1, ..., K$ in increasing order. Candidates $X_i$ are visited from least to most sensitive based on the calculated $I(X_i;X_j)$ in Step 1; at each $k$, we conducted a statistical significance test on assessing $I(X_i;Y_j|\mathbf{X}^{S_2}_j) = 0$, and $X_i$ is removed from $\mathbf{X}^{S}_j$ as soon as one such test is not significant (i.e., $I(X_i;Y_j|\mathbf{X}^{S_2}_j) = 0$), after which the candidate set used to build $\mathbf{X}^{S_2}_j$ for subsequent tests is updated accordingly.
